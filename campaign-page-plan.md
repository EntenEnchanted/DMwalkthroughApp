...
Typed blocks replace the one-blob-plus-markup approach. The classifier emits
**both** the extracted block *and* keeps the inline span in the prose — one
source of truth, two renderings. You get a scannable list without losing the
paragraph context that makes a branch make sense.
...
├─ Checks        table: skills, DC, cost, success, fail
├─ Background    lore
...
| `blocks/BackgroundBlock.tsx` | prose with retained inline markup |
...
**Phase 0 — fixture.** Hand-author B2: Fungus Farm and A5: Temple of Bahamut in
the new JSON shape, checked into the repo. No API spend, and the hand-authored
version becomes the accuracy yardstick for Phase 3.
