import type { Env, IngestSection, SectionType } from "../types.js";
import { upsertSection, embeddingText } from "../db.js";
import { embed } from "../embeddings.js";

const BATCH_SIZE = 20;

// --- Remote classify proxy ---
//
// Mirrors ingest/src/classify.ts exactly (same prompt/params) so a local
// run and a remote run (paired with ingest/src/runRemote.ts) produce
// identical results — this exists only because this sandboxed environment
// has no local ANTHROPIC_API_KEY for the ingest package's own Anthropic
// SDK usage, so classification is proxied through the deployed worker's
// secret instead. Temporary scaffolding, same as the one used for the
// original DoSI ingestion (see git history) — remove once the second
// module's ingestion is done.
...
function classifyLooksCorrupted(text: string): boolean {
  if (CLASSIFY_CORRUPTION_MARKERS.some((marker) => text.includes(marker))) return true;
...
function classifySanitize(text: string): string {
  let cut = -1;
  for (const marker of CLASSIFY_CORRUPTION_MARKERS) {
...
export async function handleClassifySection(request: Request, env: Env): Promise<Response> {
  ...
  return Response.json({
    type,
    read_aloud_text: classifySanitize(result.read_aloud_text ?? ""),
    dm_only_text: classifySanitize(result.dm_only_text ?? ""),
    reveals: result.reveals ?? [],
    creature_references: (result.creature_references ?? []).filter((n) => body.creatureNames.includes(n)),
  });
}
