...
export async function requireCampaignDm(
  ...
  const campaign = await env.DB.prepare(`SELECT id, module_id, dm_user_id FROM campaigns WHERE id = ?`)
    .bind(campaignId)
    .first<CampaignRow>();
...
export async function handleListCampaigns(request: Request, env: Env): Promise<Response> {
  ...
  const { results } = await env.DB.prepare(
    `SELECT c.id, c.name, c.module_id, m.name as module_name, c.status, c.created_at
